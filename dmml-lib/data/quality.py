...
class CheckActualityByDate(CheckDf):
    def __init__(self, name: str = None, columns: Union[str, List[str]] = None, valid_delta: relativedelta = None):
        super().__init__(name, columns)
        self.valid_delta = valid_delta

    def check_func(self, values: pd.DataFrame, column: str) -> bool:
        actuality_date = datetime.datetime.now() - self.valid_delta
        return len(values[values[column] < actuality_date]) == 0
