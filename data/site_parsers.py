...
class AvitoParser:
    ...
    def _get_items_from_tree(tree) -> pd.DataFrame:
        titles = tree.xpath(
            "//div[@data-marker='catalog-serp']//a[@data-marker='item-title']"
        )
        prices = tree.xpath(
            "//div[@data-marker='catalog-serp']//span[@data-marker='item-price']/meta[@itemprop='price']"
        )
        times = tree.xpath(
            "//div[@data-marker='catalog-serp']//div[@data-marker='item-date']"
        )
    ...
    def search(
        ...
        while next_page:
            ...
            tree = self._html_to_tree(
                self._generate_search_url_for(
                    city, category, request_, price_from, price_to, page
                )
            )
            if tree is None:
                break
            ...
            next_page = (
                len(
                    tree.xpath(
                        "//span[@data-marker='pagination-button/next' and not(contains(@class, 'readonly'))]"
                    )
                )
                > 0
            )
